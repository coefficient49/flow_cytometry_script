function varargout = flow_software_JC(varargin)
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @flow_software_JC_OpeningFcn, ...
    'gui_OutputFcn',  @flow_software_JC_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before flow_software_JC is made visible.
function flow_software_JC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to flow_software_JC (see VARARGIN)

% Choose default command line output for flow_software_JC
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes flow_software_JC wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = flow_software_JC_OutputFcn(hObject, eventdata, handles)
...
% --- Executes on button press in flow_software.
function flow_software_Callback(hObject, eventdata, handles)
% hObject    handle to flow_software (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% try
[t,targetFolder] = flow_software;
c1 = {};c2 = {};c3 = {};c4 = {};c5 = {};c6 = {};c7 = {};
try
    for x = 1:size(t,1)
        c1 = [c1;t.fileName(x,:)];
        c2 = [c2;t.percent_positive(x,:)];
        c3 = [c3;t.geomean_val(x,:)];
        c4 = [c4;t.TI(x,:)];
        c5 = [c5;t.Cells(x,:)];
        c6 = [c6;t.Virus(x,:)];
        c7 = [c7;t.MOI(x,:)];
    ...
    set(handles.uitable1,'Data',[c1 c2 c3 c4 c5 c6 c7],...
        'ColumnName',...
        [{'fileName'} {'percent_positive'} {'geomean_val'} {'TI'} {'Cells'} {'Virus'} {'MOI'}],...
        'ColumnWidth',{200,100,100,100,75,75,50})
    run flow_software_JC
    plot_variables(hObject, eventdata, handles)
catch
    for x = 1:size(t,1)
        c1 = [c1;t.fileName(x,:)];
        c2 = [c2;t.percent_positive(x,:)];
    ...
    set(handles.uitable1,'Data',[c1 c2 c3 c4],...
        'ColumnName',...
        [{'fileName'} {'percent_positive'} {'geomean_val'} {'TI'}],...
...
try
    ...
    msgbox('no MOI, Virus, Cells data found!')
end
...
function plot_variables(hObject, eventdata, handles)
%% for ploting
t_data =get(handles.uitable1,'data');
t_name = get(handles.uitable1,'ColumnName');

t = cell2table(t_data);
t.Properties.VariableNames = t_name;
selection = get(handles.popupmenu1,'Value');
switch selection
    case 1
        
        summary = varfun(@mean,t,'InputVariables','TI',...
            'GroupingVariables','MOI');
        err = varfun(@std,t,'InputVariables','TI',...
            'GroupingVariables','MOI');
        
        axes(handles.axes1)
        bar(summary.MOI,summary.mean_TI)
        hold on
        er=errorbar(summary.MOI,summary.mean_TI,err.std_TI);
        er.Color = [0 0 0];
        er.LineStyle = 'none';
        hold off
        xlabel('MOI')
    ...
    case 2
        summary = varfun(@mean,t,'InputVariables','TI',...
            'GroupingVariables','Virus');
        err = varfun(@std,t,'InputVariables','TI',...
            'GroupingVariables','Virus');
        
        axes(handles.axes1)
        bar(summary.mean_TI)
        set(gca,'xticklabel',summary.Virus);
        hold on
        er=errorbar(summary.mean_TI,err.std_TI);
        er.Color = [0 0 0];
        er.LineStyle = 'none';
        hold off
        xlabel('Virus')
    ...
    case 3
        summary = varfun(@mean,t,'InputVariables','TI',...
            'GroupingVariables','Cells');
        err = varfun(@std,t,'InputVariables','TI',...
            'GroupingVariables','Cells');
        
        axes(handles.axes1)
        bar(summary.mean_TI)
        set(gca,'xticklabel',summary.Cells);
        hold on
        er=errorbar(summary.mean_TI,err.std_TI);
        er.Color = [0 0 0];
        er.LineStyle = 'none';
...
function [tbl_Data,pathName]=flow_software(handles)

[fileName,pathName]=uigetfile(".fcs");

answer = questdlg('Would you like to load previous gating settings?', ...
    'load gating?', ...
    'Yes, load a previous gating info','No, I will establish a new gate','No, I will establish a new gate');
% Handle response
switch answer
    case 'Yes, load a previous gating info'
        newGate = false;
    case 'No, I will establish a new gate'
        newGate = true;
end
[fcsdat, fcshdr, fcsdatscaled, fcsdat_comp] = fca_readfcs([pathName,fileName]);
a=figure(121);
set(a,'units','normalized','outerposition',[0 0 0.5 0.5])
fcsdat_table = array2table(fcsdat);
fcsdat_table.Properties.VariableNames = {'FSC' 'SSC' 'FITC' 'PE' 'PerCP_Cy5' 'PE_Cy7' 'APC' 'APC_Cy7' 'Time' };
plot(fcsdat_table.FSC,fcsdat_table.SSC,'.','markersize',0.1)
...
if newGate
    ...
    while keep_going
        try
            V = ginput(1);
            vertices=[vertices;V];
            plot(fcsdat_table.FSC,fcsdat_table.SSC,'.',vertices(:,1),vertices(:,2),'r*-')
        catch
            keep_going=0;
        end
    ...
    K = inpolygon(fcsdat_table.FSC,fcsdat_table.SSC,vertices(:,1),vertices(:,2));
else
    [gateFileName,gatePathName]=uigetfile("*.mat");
    loadedVariable = load([gatePathName,gateFileName]);
    vertices=loadedVariable.vertices;
    plot(fcsdat_table.FSC,fcsdat_table.SSC,'.',vertices(:,1),vertices(:,2),'r*-')
    K = inpolygon(fcsdat_table.FSC,fcsdat_table.SSC,vertices(:,1),vertices(:,2));
end
cd(pathName)
save("gateInfo.mat",'vertices')
ff = fcsdat_table.FITC(K);
good_ff = ff(ff>0);
b=figure(122);
[N,X]=hist(log10(good_ff),linspace(0,5,200));
CDF_ = cumsum(N/sum(N));
PDF_ = N/max(N);
th = min(find(CDF_>(1-0.001)));
% plot(X,PDF_,X,CDF_,[X(th) X(th)],[0 1])
plot(X,PDF_,[X(th) X(th)],[0 1])
axis([0 6 0 1])
title('FITC-gating at 0.1% of negative population')

filenames = dir([pathName '*fcs']);
rows = floor(sqrt(2*length(filenames)));
cols = floor(sqrt(2*length(filenames)));
while rows*cols < 2*length(filenames)
    cols = cols+1;
end
%%

c = figure(133);
set(c,'units','normalized','outerposition',[0 0.5 0.5 0.5])


data_ = struct('fileName',[],'percent_positive',[],'geomean_val',[],'TI',[]);
h = waitbar(0,'loading flow files');
for namesF = 1:length(filenames)
    
    [fcsdat, fcshdr, fcsdatscaled, fcsdat_comp] = fca_readfcs(filenames(namesF).name);
    fcsdat_table = array2table(fcsdat);
    fcsdat_table.Properties.VariableNames = {'FSC' 'SSC' 'FITC' 'PE' 'PerCP_Cy5' 'PE_Cy7' 'APC' 'APC_Cy7' 'Time' };
    K = inpolygon(fcsdat_table.FSC,fcsdat_table.SSC,vertices(:,1),vertices(:,2));
    ff = fcsdat_table.FITC(K);
    ...
    subplot(rows,cols,2*namesF-1)
    plot(fcsdat_table.FSC,fcsdat_table.SSC,'.',fcsdat_table.FSC(K),fcsdat_table.SSC(K),'.','markersize',0.1)
    t= title(filenames(namesF).name);
    set(t,'Interpreter', 'none')
    
    subplot(rows,cols,2*namesF)
    [N,X]=hist(log10(good_ff),linspace(0,5,200));
    PDF_ = N/max(N);
    plot(X,PDF_,[X(th) X(th)],[0 1])
    t= title(filenames(namesF).name);
    ...
    % fcsdat_table.Properties.VariableNames = {'FSC' 'SSC' 'FITC' 'PE' 'PerCP_Cy5' 'PE_Cy7' 'APC' 'APC_Cy7' 'Time' };
    % ii=1;
    % for x =  {'FSC' 'SSC' 'FITC' 'PE' 'PerCP_Cy5' 'PE_Cy7' 'APC' 'APC_Cy7' 'Time' }
    %     subplot(3,3,ii)
    %     eval(strjoin(["ff=fcsdat_table." char(x) ';'],''));
    % hist(log10(ff(ff>0)),500)
    % title(x)
    % ii=ii+1;
    % end
    waitbar(namesF/length(filenames),h)
    percent_positive = nnz(good_ff > 10^X(th))/numel(good_ff)*100;
    geomean_val = geomean(good_ff(good_ff > 10^X(th)));
    TI = geomean_val*percent_positive;
    data_.percent_positive = [data_.percent_positive;percent_positive];
    data_.geomean_val = [data_.geomean_val;geomean_val];
    data_.TI = [data_.TI;TI];
    data_.fileName = [data_.fileName ; char(filenames(namesF).name)];
end
close(h)
figure(122)
close
...
close
% close(c)
tbl_Data = struct2table(data_);
try
    sampleID=readtable('sampleID.xlsx');
    tbl_Data = [tbl_Data sampleID];
    writetable(tbl_Data,'flowdata.csv');
catch
    try
        sampleID=readtable('sampleID.txt');
        tbl_Data = [tbl_Data sampleID];
        writetable(tbl_Data,'flowdata.csv');
    catch
        writetable(tbl_Data,'flowdata.csv');
        msgbox(sprintf('No sampleID.xlsx or sampleID.txt found!\nOnly raw data saved.'))
    end
end
